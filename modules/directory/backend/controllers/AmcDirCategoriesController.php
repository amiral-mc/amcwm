...
class AmcDirCategoriesController extends BackendController {
     ...
     * @param DirCategories $model
     * @access protected
     */
    protected function save(DirCategoriesTranslation $contentModel) {
        if (isset($_POST['DirCategories']) && isset($_POST["DirCategoriesTranslation"])) {
            $transaction = Yii::app()->db->beginTransaction();
            $model = $contentModel->getParentContent();
            $model->attributes = $_POST['DirCategories'];
            $contentModel->attributes = $_POST['DirCategoriesTranslation'];
    ...
    public function actionCreate() {
        $model = new DirCategories;
        $contentModel = new DirCategoriesTranslation();
    ...
    public function actionTranslate($id) {
        ...
        if ($contentModel) {
            ...
            if (isset($_POST["DirCategoriesTranslation"])) {
                $translatedModel->attributes = $_POST['DirCategoriesTranslation'];
    ...
    public function actionIndex() {
        $model = new DirCategories('search');
        $model->unsetAttributes();  // clear any default values
        $model->unsetTranslationsAttributes();  // clear any default values
        $model->addTranslationChild(new DirCategoriesTranslation('search'), self::getContentLanguage());
        $contentModel = $model->getTranslated(self::getContentLanguage());
        if (isset($_GET['DirCategories'])) {
            $model->attributes = $_GET['DirCategories'];
        }
        if ($contentModel) {
            if (isset($_GET['DirCategoriesTranslation'])) {
                $contentModel->attributes = $_GET['DirCategoriesTranslation'];
            }
    ...
    public function loadModel($id) {
        $model = DirCategories::model()->findByPk($id);
        if ($model === null)
            throw new CHttpException(404, 'The requested page does not exist.');
        return $model;
    }
    ...
    public function loadTranslatedModel($model, $id) {
        ...
        if ($model === null) {
            ...
            $translatedModel = DirCategoriesTranslation::model()->findByPk(array("category_id" => (int) $id, 'content_lang' => $translationLang));
            if ($translatedModel === null) {
                $translatedModel = new DirCategoriesTranslation();
    ...
    public function loadChildModel($id) {
        ...
        $model = DirCategoriesTranslation::model()->findByPk(array("category_id" => $pk['id'], 'content_lang' => $pk['lang']));
        if ($model === null)
            throw new CHttpException(404, 'The requested page does not exist.');
        return $model;
    }
