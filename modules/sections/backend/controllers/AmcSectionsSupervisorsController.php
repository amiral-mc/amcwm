 ...
 * @version 1.0
 */
class AmcSectionsSupervisorsController extends BackendController {
                ...
                try {
                    if ($model->save()) {
                        if ($contentModel->save()) {
                            $transaction->commit();
                            Yii::app()->user->setFlash('success', array('class' => 'flash-success', 'content' => AmcWm::t("msgsbase.core", 'Supervisor data has been saved')));
    ...
    public function actionDelete() {
        ...
        if (Yii::app()->request->isPostRequest && count($ids)) {
            ...
            foreach ($ids as $id) {
                $contentModel = $this->loadChildModel($id);
                $model = $contentModel->getParentContent();
                $checkRelated = (count($model->sections) || count($model->users) || count($model->writers));
                if ($checkRelated) {
                    $messages['error'][] = AmcWm::t("msgsbase.core", 'Can not delete supervisor "{supervisor}"', array("{supervisor}" => $contentModel->name));
                } else {
                    $model->delete();
                    $messages['success'][] = AmcWm::t("msgsbase.core", 'Supervisor "{supervisor}" has been deleted', array("{supervisor}" => $contentModel->name));
                }
            }
    ...
    public function loadModel($id) {
        $model = Persons::model()->findByPk((int) $id);
        if ($model === null)
            throw new CHttpException(404, 'The requested page does not exist.');
        return $model;
    }
