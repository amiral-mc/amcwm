 ...
 * @version 1.0
 */
class AmcWritersController extends BackendController {
            ...
            if ($validate) {
                ...
                if ($success) {
                   $model->saveImage($deleteImageFile);
                    Yii::app()->user->setFlash('success', array('class' => 'flash-success', 'content' => AmcWm::t("msgsbase.core", 'Writer data has been saved')));
        ...
        if (Yii::app()->request->isPostRequest && count($ids)) {
            ...
            foreach ($ids as $id) {
                ...
                if ($checkRelated) {
                    ...
                    $deleted = $model->writers->delete();
                    if ($deleted) {
                        $deleted = $model->delete();
                        if ($deleted) {
                            $model->deleteImage();
                            $messages['success'][] = AmcWm::t("msgsbase.core", 'Writer "{writer}" has been deleted', array("{writer}" => $contentModel->name));
                        }
                    }
                }
            }
    ...
    public function loadModel($id) {
        $writer = Writers::model()->findByPk((int) $id);
