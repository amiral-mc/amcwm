...
if ($model->tender_id && $model->file_ext && is_file(str_replace("/", DIRECTORY_SEPARATOR, Yii::app()->basePath . "/../" . $mediaSettings['paths']['files']['path'] . "/" . $model->tender_id . "." . $model->file_ext))) {
    $drawDocLink = '<a href="' . Html::createUrl('/site/download', array('f' => "{$mediaSettings['paths']['files']['path']}/{$model->tender_id}.{$model->file_ext}")) . '">' . AmcWm::t("msgsbase.core", 'Download the file') . '</a>';
}
