...
class TbSelect2 extends CInputWidget {
    ...
    public function registerClientScript($id) {
        Yii::app()->bootstrap->assetsRegistry->registerPackage('select2');
    ...
    public function registerLanguageScript() {
        if (isset($this->options['language']) && $this->options['language'] != 'en') {
            $file = 'locales/select2_locale_' . $this->options['language'] . '.js';
            if (@file_exists(Yii::getPathOfAlias('bootstrap.assets.select2') . '/js/' . $file)) {
                $name = 'select2_locale_' . $this->options['language'] . '.js';
                Yii::app()->bootstrap->assetsRegistry->registerScriptFile(Yii::app()->bootstrap->getAssetsUrl() . "/select2/js/locales/{$name}", CClientScript::POS_END);
            }
        }
    }
