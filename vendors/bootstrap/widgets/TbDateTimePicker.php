...
class TbDateTimePicker extends CInputWidget {
    ...
    public function registerClientScript() {
        Yii::app()->bootstrap->assetsRegistry->registerPackage('datetimepicker');
    }
    ...
    public function registerLanguageScript() {
        if (isset($this->options['language']) && $this->options['language'] != 'en') {
            ...
            if (@file_exists(Yii::getPathOfAlias('bootstrap.assets.bootstrap-datetimepicker') . '/js/' . $file)) {
                $name = 'bootstrap-datetimepicker.' . $this->options['language'] . '.js';
               Yii::app()->bootstrap->assetsRegistry->registerScriptFile(Yii::app()->bootstrap->getAssetsUrl() . "/bootstrap-datetimepicker/js/locales/{$name}", CClientScript::POS_END);
            }
        }
    }
